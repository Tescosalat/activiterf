...
export const MapGoogle = ({name, photo}) => {
  ...
  const [activityMenu, setActivityMenu] = useState("hide")
...
const handleAddToChat = (chatPartner) => {
  if (chatPartners.includes(chatPartner)) return router.push(`/profile/${name}/chatMenu/${chatPartner}`)

    socket.emit("private_message", { to: chatPartner, message: `${name} wants to join` });
...
if (!timeTemp) return alert("time is missing")
      ...
      setTimeTemp("")
      setIconTemp(null)
      setActivityMenu("hide")
      getAllMarkers()
}

const onMapClick = (e) => {

    setActivityMenu("show")
  ...
  return (
    <APIProvider apiKey={process.env.NEXT_PUBLIC_MAPS}>
      ...
      <button className="exit-button" onClick={() => setActivityMenu("hide")}><IoCloseOutline /></button>
        <div className="icon-container-activity">
        {icons.map((icon, index) => {
            return ( 
                <button className={index === iconTemp ? "selected" : ""} onClick={() => setIconTemp(icons.indexOf(icon))} key={index}>{icons[icons.indexOf(icon)]}</button>
            )
        })}
        ...
        <div className="time"><h1>When ?</h1><input onChange={(e) => setTimeTemp(e.target.value)} className="time-input" type="date" /></div>
        <h1>Tell the world about your plans:</h1>
        <textarea value={textTemp} onChange={(e) => setTextTemp(e.target.value)} className="describe-field"/>
      ...
      <div style={{ height: "100%", width: "100%" }}>
        {currentPosition ? (
          ...
          >
            <AdvancedMarker position={currentPosition} gmpDraggable={false}>
              <Pin><p style={{fontSize: "12px"}}>Me</p></Pin>
            </AdvancedMarker>
        ...
        <AdvancedMarker 
          ...
          {marker.open && <InfoWindow onCloseClick={() => toggleInfoWindow(index)} position={{ lat: marker.lat,
            lng: marker.lng }}>
